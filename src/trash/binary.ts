/**
 * Take a list xs and a value v such that xs is sorted and v is comparable
 * with the elements of xs.
 * 
 * Return an index i such that 0 <= i <= len(xs) with the properties:
 * 
 * 1. xs.splice(i, 0, v) is sorted
 * 2. xs.splice(j, 0, v) is not sorted for j < i
 */
function binarySearch<T>(xs: T[], v: T): number {
 ...
 * def binary_search(xs, v):
    ...
    while lo + 1 < hi:
        ...
        #   return mid
        exse:
            # Either v == xs[mid] in which case mid is a valid insertion point
            # or v < xs[mid], in which case all valid insertion points must be
...
# We could axso do it this way, but that would be a bad idea:
...
@given(xs=SortedLists, v=Values)
def test_insert_is_sorted(xs, v):
    ...
    set."""
    xs.insert(binary_search(xs, v), v)
    assert is_sorted(xs)


@given(xs=SortedLists, v=Values)
def test_is_minimal(xs, v):
    ...
    for i in range(binary_search(xs, v)):
        xs2 = list(xs)
        xs2.insert(i, v)
        assert not is_sorted(xs2)


@given(xs=SortedLists, v=Values)
def test_inserts_into_same_place_twice(xs, v):
    ...
    """
    i = binary_search(xs, v)
    xs.insert(i, v)
    assert binary_search(xs, v) == i
 */